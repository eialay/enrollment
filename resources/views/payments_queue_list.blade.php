<x-sidebar>
    ...
    <div class="max-w-6xl mx-auto mt-10 px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-bold text-gray-800">💳 Payment Queue</h2>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($queueList as $queue)
                        <tr class="hover:bg-blue-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $queue->student->user->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $queue->payment_reference_code ?? '—' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 flex justify-center gap-3">
                                @if($queue->payment)
                                    <a href="{{ route('payments.details', $queue->payment->id) }}" 
                                ...
                                @endif
                                <form method="POST" action="{{ route('payments.queueDelete', $queue->id) }}" 
